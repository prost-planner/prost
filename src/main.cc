...
void printUsage() {
    ...
    cout << "Originally, the PROST planner has been the planning system that won IPPC 2011. It has been extended step by step and is now a framework that implements several Trial-based Heuristic Tree Search Algorithms including UCT, DP-UCT and UCT* as described by Keller and Helmert (2013)." 
         << endl << "We aim to develop a system where components (i.e., search engines) can be mixed and matched at will. Therefore, option values of search engines in the following are either primitive or they can be a search engine themselves. Note that by far not each cmbination has been tested." << endl << endl;
    ...
    cout << "  -bit <32 | 64>" << endl;
    cout << "    Specifies the system's bit size. Should not be set by hand." << endl;
    ...
    cout << "An MC-UCT search engine is equivalent to the algorithm described by Kocsis and Szepesvari (2006), and is implemented in terms of the Trial-based Heuristic Tree Search framework. It can be created as [MC-UCT <options>] with the following options:" 
         << endl << endl;

    cout << "  -uc <0|1>" << endl;
    cout << "    Specifies if caching is used. If this is switched on, we keep track of the memory and stop caching once a critical amount of memory is used." 
         << endl;
    cout << "    Default: 1" << endl << endl;

    cout << "  -task <PROB | MLD>" << endl;
    cout << "    Specifies if the probabilistic task (PROB) or the most likely determinized (MLD) is used."
         << endl;
    ...
    cout << "  -r <int>" << endl;
    cout << "    Specifies the maximal number of trials. This must be set if used as the default is 0." << endl;
    cout << "    Default: 0" << endl << endl;

    cout << "  -ndn <int>" << endl;
    cout << "    Specifies the number of previously unvisited decision nodes that is expanded before the trial length is sufficient." << endl;
    cout << "    Default: Horizon of task (i.e., the trial length is only sufficient in states with no remaining steps." << endl << endl;
    ...
    cout << "    Specifies the search engine that is used for initialization." << endl;
    cout << "    Default: n/a, this MUST be specified." << endl << endl << endl;
    ...
    cout << "An MaxMC-UCT search engine is equivalent to the algorithm described by Keller and Helmert(2013), and is implemented in terms of the Trial-based Heuristic Tree Search framework. It can be created as [MaxMC-UCT <options>] with the following options:" 
         << endl << endl;
    ...
    cout << "An DP-UCT search engine is equivalent to the algorithm described by Keller and Helmert(2013), and is implemented in terms of the Trial-based Heuristic Tree Search framework. It can be created as [DP-UCT <options>] with the following options:" 
         << endl << endl;
    ...
    cout << "/*************************************************************" << endl
        ...
        cout << "An IDS search engine is equivalent to the search engine that is described by Keller and Eyerich (2012) as their initialization method. It can be created as [IDS <options>] with the following options:"
         << endl << endl;
    ...
    cout << "  -sd <int>" << endl;
    cout << "    Specifies the considered horizon." << endl;
    cout << "    Default: Learned based on timeout and strict timeout." << endl << endl;
    ...
    cout << "A DFS search engine is a depth first search engine that has only been tested as part of IDS. It can be created as [DFS <options>] with the following options:"
         << endl << endl;
    ...
    cout << "Uniform Evaluation Search evaluates each action identical. It can be created as [Uniform <options>] with the following options:"
         << endl << endl;

    cout << "  -task <PROB | MLD>" << endl;
    cout << "    Specifies if the probabilistic task (PROB) or the most likely determinized (MLD) is used. If you run this standalone (as a random search engine), use PROB, otherwise, use DET (to achieve correct pruning behaviour)."
         << endl;
    cout << "    Default: MLD" << endl << endl;

    cout << "  -val <double | INFTY>" << endl;
    cout << "    Specifies the estimate assigned to each action. If this is INFTY, it is set to the maximal reward of the task and is thus a (very simple) admissible initialization." << endl << endl << endl;

    cout << "/*************************************************************" << endl
         << "                         Abbreviations" << endl
         << "*************************************************************/" << endl;

    cout << "There are also some short cuts to quickly use search engines that have been used quite often:" << endl << endl;
    cout << "  [IPPC2011] := [MC-UCT -sd 15 -i [IDS -sd 15]]" << endl;
    cout << "    This corresponds to the planner that has been used for IPPC 2011 (except that all bugfixes are considered and that the timemanagement framework that had been used is not part of this." << endl << endl;
