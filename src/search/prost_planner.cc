...
void ProstPlanner::init() {
    ...
    if (ProbabilisticSearchEngine::useRewardLockDetection &&
        ...
        for (unsigned int index = 0; index < SearchEngine::allCPFs.size();
             ++index) {
...
vector<string> ProstPlanner::plan(vector<double> const& nextStateVec) {
    ...
    for (unsigned int i = 0;
         i <
         SearchEngine::actionStates[chosenActionIndex].scheduledActionFluents.
         size();
         ++i) {
        result.push_back(
                SearchEngine::actionStates[chosenActionIndex].
...
void ProstPlanner::monitorRAMUsage() {
    if (cachingEnabled && (SystemUtils::getRAMUsedByThis() > ramLimit)) {
        ...
        for (unsigned int i = 0; i < SearchEngine::actionPreconditions.size();
             ++i) {
...
void ProstPlanner::initNextStep(vector<double> const& nextStateVec) {
    ...
    currentState =
        State(nextValuesOfDeterministicStateFluents,
                nextValuesOfProbabilisticStateFluents,
                remainingSteps);
