...
LogicalExpression* LogicalExpression::createFromString(vector<StateFluent*> const& stateFluents, vector<ActionFluent*> const& actionFluents, string& desc) {
    if(StringUtils::startsWith(desc, "$s(")) {
        ...
        LogicalExpression* expr = LogicalExpression::createFromString(stateFluents, actionFluents, desc);
        ...
        for(unsigned int index = 0; index < tokens.size(); ++index) {
            ...
            values.push_back(LogicalExpression::createFromString(stateFluents, actionFluents, valProbPair[0]));
            probabilities.push_back(LogicalExpression::createFromString(stateFluents, actionFluents, valProbPair[1]));
        }
        ...
        for(unsigned int index = 0; index < tokens.size();) {
            ...
            conditions.push_back(LogicalExpression::createFromString(stateFluents, actionFluents, tokens[index]));
            ++index;

            tokens[index] = tokens[index].substr(5,tokens[index].length()-6);
            effects.push_back(LogicalExpression::createFromString(stateFluents, actionFluents, tokens[index]));
            ++index;
        }
...
void LogicalExpression::createExpressions(vector<StateFluent*> const& stateFluents, vector<ActionFluent*> const& actionFluents, string& desc, vector<LogicalExpression*>& exprs) {
    ...
    for(unsigned int index = 0; index < tokens.size(); ++index) {
        exprs.push_back(LogicalExpression::createFromString(stateFluents, actionFluents, tokens[index]));
    }
}
