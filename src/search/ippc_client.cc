...
void IPPCClient::readState(const XMLNode* node, vector<double>& nextState) {
    ...
    for (map<string, string>::iterator it = newValues.begin();
         it != newValues.end(); ++it) {
        if (stateVariableIndices.find(it->first) !=
            ...
            if (stateVariableValues[stateVariableIndices[it->first]].empty()) {
                ...
                if (it->second == "true") {
                    ...
                    nextState[stateVariableIndices[it->first]] = atof(
                            it->second.c_str());
                }
            } else {
                for (unsigned int i = 0;
                     i <
                     stateVariableValues[stateVariableIndices[it->first]].size();
                     ++i) {
                    if (stateVariableValues[stateVariableIndices[it->first]][i]
                        == it->second) {
