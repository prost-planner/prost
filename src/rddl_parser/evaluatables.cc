...
void Evaluatable::simplify(map<StateFluent*, double>& replacements) {
    ...
    for (map<StateFluent*, double>::iterator it = replacements.begin();
         it != replacements.end(); ++it) {
        if (dependentStateFluents.find(it->first) !=
...
void Evaluatable::initializeKleeneStateFluentHashKeys(
        vector<ConditionalProbabilityFunction*> const& CPFs,
        vector<vector<pair<int,
                           long> > >& indexToKleeneStateFluentHashKeyMap,
        long const& firstStateFluentHashKeyBase) {
    long nextHashKeyBase = firstStateFluentHashKeyBase;
    ...
    for (unsigned int index = 0; index < tmpStateFluentDependencies.size();
         ++index) {
        indexToKleeneStateFluentHashKeyMap[tmpStateFluentDependencies[index].
                                           first].push_back(make_pair(hashIndex,
                        tmpStateFluentDependencies[index].second));
    }
