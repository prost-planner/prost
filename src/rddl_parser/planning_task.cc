...
ActionFluent* PlanningTask::getActionFluent(string const& name) {
    ...
    SystemUtils::abort(
            "Error: action-fluent " + name + " used but not defined.");
    return NULL;
}
...
vector<StateFluent*> PlanningTask::getVariablesOfSchema(
...
void PlanningTask::addCPF(ConditionalProbabilityFunction* const& cpf) {
    for (unsigned int i = 0; i < CPFs.size(); ++i) {
        if (cpf->head->fullName == CPFs[i]->head->fullName) {
            SystemUtils::abort(
                    "Error: CPF with same name exists already: " +
                    cpf->head->fullName);
        }
    }
    CPFs.push_back(cpf);
...
void PlanningTask::print(ostream& out) {
    ...
    "## detected during task analysis, and the total number of encountered states"
        << endl;
    out << nonTerminalStatesWithUniqueAction << " " <<
    numberOfEncounteredStates << endl;
