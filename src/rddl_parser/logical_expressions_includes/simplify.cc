LogicalExpression* LogicalExpression::simplify(map<StateFluent*,
                                                   double>& /*replacements*/) {
    print(cout);
    assert(false);
    return NULL;
...
LogicalExpression* ParametrizedVariable::simplify(map<StateFluent*,
                                                      double>& /*replacements*/)
{
    return this;
}

LogicalExpression* StateFluent::simplify(map<StateFluent*,
                                             double>& replacements) {
...
LogicalExpression* NumericConstant::simplify(map<StateFluent*,
                                                 double>& /*replacements*/) {
    return this;
}
...
LogicalExpression* Conjunction::simplify(map<StateFluent*,
                                             double>& replacements) {
    ...
    for (unsigned int i = 0; i < exprs.size(); ++i) {
        ...
        if (nc) {
            ...
            if (conj) {
                newExprs.insert(newExprs.end(),
                        conj->exprs.begin(), conj->exprs.end());
            } else {
...
LogicalExpression* Disjunction::simplify(map<StateFluent*,
                                             double>& replacements) {
    vector<LogicalExpression*> newExprs;
    for (unsigned int i = 0; i < exprs.size(); ++i) {
        LogicalExpression* newExpr = exprs[i]->simplify(replacements);
        ...
        if (nc) {
            ...
            if (disj) {
                newExprs.insert(newExprs.end(),
                        disj->exprs.begin(), disj->exprs.end());
            } else {
                newExprs.push_back(newExpr);
            }
        }
...
LogicalExpression* EqualsExpression::simplify(map<StateFluent*,
                                                  double>& replacements) {
    vector<LogicalExpression*> newExprs;
...
LogicalExpression* GreaterExpression::simplify(map<StateFluent*,
                                                   double>& replacements) {
    assert(exprs.size() == 2);
...
LogicalExpression* LowerExpression::simplify(map<StateFluent*,
                                                 double>& replacements) {
    assert(exprs.size() == 2);
    LogicalExpression* expr0 = exprs[0]->simplify(replacements);
    LogicalExpression* expr1 = exprs[1]->simplify(replacements);
...
LogicalExpression* GreaterEqualsExpression::simplify(map<StateFluent*,
                                                         double>& replacements)
{
    assert(exprs.size() == 2);
    LogicalExpression* expr0 = exprs[0]->simplify(replacements);
...
LogicalExpression* LowerEqualsExpression::simplify(map<StateFluent*,
                                                       double>& replacements) {
    assert(exprs.size() == 2);
    LogicalExpression* expr0 = exprs[0]->simplify(replacements);
    LogicalExpression* expr1 = exprs[1]->simplify(replacements);
...
LogicalExpression* Addition::simplify(map<StateFluent*,
                                          double>& replacements) {
    vector<LogicalExpression*> newExprs;
    double constSum = 0.0;
    ...
    for (unsigned int i = 0; i < newExprs.size(); ++i) {
        ...
        if (add) {
            finalExprs.insert(finalExprs.end(),
                    add->exprs.begin(), add->exprs.end());
            //if the merged addition had an constant element it must have been the last!
            NumericConstant* nc =
                dynamic_cast<NumericConstant*>(finalExprs[finalExprs.size() -
                                                          1]);
            if (nc) {
                constSum += nc->value;
                finalExprs.pop_back();
...
LogicalExpression* Subtraction::simplify(map<StateFluent*,
                                             double>& replacements) {
    assert(exprs.size() >= 2);
    vector<LogicalExpression*> newExprs;
...
LogicalExpression* Multiplication::simplify(map<StateFluent*,
                                                double>& replacements) {
    vector<LogicalExpression*> newExprs;
...
LogicalExpression* Division::simplify(map<StateFluent*,
                                          double>& replacements) {
    assert(exprs.size() == 2);
    LogicalExpression* expr0 = exprs[0]->simplify(replacements);
    LogicalExpression* expr1 = exprs[1]->simplify(replacements);
...
LogicalExpression* Negation::simplify(map<StateFluent*,
                                          double>& replacements) {
...
LogicalExpression* KronDeltaDistribution::simplify(map<StateFluent*,
                                                       double>& replacements) {
    return expr->simplify(replacements);
}

LogicalExpression* BernoulliDistribution::simplify(map<StateFluent*,
                                                       double>& replacements) {
    LogicalExpression* newExpr = expr->simplify(replacements);
    NumericConstant* nc = dynamic_cast<NumericConstant*>(newExpr);
    if (nc) {
        if (MathUtils::doubleIsEqual(nc->value, 0.0)) {
            return new NumericConstant(0.0);
        } else if (MathUtils::doubleIsGreaterOrEqual(nc->value,
                           1.0) ||
...
// true?
LogicalExpression* DiscreteDistribution::simplify(map<StateFluent*,
                                                      double>& replacements) {
...
LogicalExpression* IfThenElseExpression::simplify(map<StateFluent*,
                                                      double>& replacements) {
    ...
    if (ncTrue && ncFalse) {
        if (MathUtils::doubleIsEqual(ncTrue->value,
                    1.0) && MathUtils::doubleIsEqual(ncFalse->value, 0.0)) {
            return newCondition;
        } else if (MathUtils::doubleIsEqual(ncTrue->value,
                           0.0) &&
...
LogicalExpression* MultiConditionChecker::simplify(map<StateFluent*,
                                                       double>& replacements) {
