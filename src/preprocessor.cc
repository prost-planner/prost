...
PlanningTask* Preprocessor::preprocess(map<string, int>& stateVariableIndices, vector<vector<string> >& stateVariableValues) {
    ...
    State initialState(task->CPFs.size(), task->horizon, task->CPFs.size() + task->dynamicSACs.size() + 1);
    ...
    return new PlanningTask(actionFluents,
                            ...
                            task->dynamicSACs,
                            task->staticSACs,
                            task->stateInvariants,
                            initialState,
                            task->numberOfConcurrentActions,
                            task->horizon,
                            task->discountFactor,
                            task->noopIsOptimalFinalAction,
...
void Preprocessor::prepareActions(vector<ActionFluent*>& actionFluents) {
    ...
    for(unsigned int i = 0; i < task->CPFs.size(); ++i) {
        current[i] = task->CPFs[i]->getInitialValue();
    }
    ...
    for(list<vector<int> >::iterator it = actionCombinations.begin(); it != actionCombinations.end(); ++it) {
        ...
        for(unsigned int i = 0; i < task->staticSACs.size(); ++i) {
            double res = 0.0;
            task->staticSACs[i]->evaluate(res, current, actionState);
...
void Preprocessor::calculateDomains() {
    ...
    task->rewardCPF->setDomain(rewardDomain);
}
...
void Preprocessor::determinize() {
    ...
    for(unsigned int index = 0; index < task->CPFs.size(); ++index) {
        if(task->CPFs[index]->isProbabilistic()) {
            task->CPFs[index]->determinizedFormula = task->CPFs[index]->formula->determinizeMostLikely(randomNumberReplacement);
            task->CPFs[index]->determinizedFormula = task->CPFs[index]->determinizedFormula->simplify(replacements);
        } else {
            task->CPFs[index]->determinizedFormula = task->CPFs[index]->formula;
        }
    }
}
