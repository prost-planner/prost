...
SINGLE_PATTERNS = [
    ...
    ('ids_learned_search_depth', 'THTS heuristic IDS: Setting max search depth to: (.+)\n', int),
]

# List of repeated patterns (list attributes)
REPEATED_PATTERNS = [
    ('round_reward', '>>> END OF ROUND .* -- REWARD RECEIVED: (.+)\n', float),
    ('entries_prob_state_value_cache', 'Entries in probabilistic state value cache: (.+)\n', int),
    ('buckets_prob_state_value_cache', 'Buckets in probabilistic state value cache: (.+)\n', int),
    ('entries_prob_applicable_actions_cache', 'Entries in probabilistic applicable actions cache: (.+)\n', int),
    ('buckets_prob_applicable_actions_cache', 'Buckets in probabilistic applicable actions cache: (.+)\n', int),
    ('rem_steps_first_solved_state', 'Number of remaining steps in first solved state: (.+)\n', int),
    ('trial_initial_state', 'Number of trials in initial state: (.+)\n', int),
    ('search_nodes_initial_state', 'Number of search nodes in initial state: (.+)\n', int),
    ('perc_exploration_initial_state', 'Percentage exploration in initial state: (.+)\n', float),
    ('entries_det_state_value_cache', 'Entries in deterministic state value cache: (.+)\n', int),
    ('buckets_det_state_value_cache', 'Buckets in deterministic state value cache: (.+)\n', int),
    ('entries_det_applicable_actions_cache', 'Entries in deterministic applicable actions cache: (.+)\n', int),
    ('buckets_det_applicable_actions_cache', 'Buckets in deterministic applicable actions cache: (.+)\n', int),
    ('entries_ids_reward_cache', 'Entries in IDS reward cache: (.+)\n', int),
    ('buckets_ids_reward_cache', 'Buckets in IDS reward cache: (.+)\n', int),
    ('ids_avg_search_depth_initial_state', 'Average search depth in initial state: (.+)\n', float),
